...
export function RepVentasVendedor({ noCia, punto, ano, mes }: Props) {
  ...
  const exportPdf = async () => {
    ...
    <title>RFAT302 - Ventas por Vendedor</title>
    <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .bar{background:#4f81bd;height:8px;display:inline-block;vertical-align:middle}
    ...
    <th>Vendedor</th><th>Nombre</th><th class="r">Facturas</th>
    <th class="r">Total Neto</th><th class="r">ITBIS</th><th class="r">Total Bruto</th><th>Participacion</th></tr></thead>
    <tbody>${rows.map(r => `<tr>
    ...
    <td><span class="bar" style="width:${Math.round((r.total_bruto/maxBruto)*120)}px"></span> ${totalBruto > 0 ? ((r.total_bruto/totalBruto)*100).toFixed(1) : 0}%</td>
    </tr>`).join('')}
    </tbody>
    <tfoot><tr><td colspan="2"><b>TOTALES — ${rows.length} vendedor(es)</b></td>
    <td class="r"><b>${totalFacts}</b></td>
    <td class="r"><b>${fmtN(totalNeto)}</b></td>
    <td class="r"><b>${fmtN(totalItbis)}</b></td>
    <td class="r"><b>${fmtN(totalBruto)}</b></td><td></td></tr></tfoot>
    </table></body></html>`)
      ...
      <div className='flex flex-wrap items-center justify-between gap-3'>
        <div>
          <h2 className='text-lg font-semibold flex items-center gap-2'>
            <Users className='h-5 w-5' /> Ventas por Vendedor
          </h2>
        ...
        <TableBody>
          ...
          {rows.map((row) => (
            <TableRow key={row.vendedor}>
              ...
              <TableCell className='text-sm font-medium'>{row.nombre_vendedor}</TableCell>
