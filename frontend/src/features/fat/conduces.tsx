...
export function ConducesFat({ noCia, punto, ano, mes }: Props) {
  ...
  const exportCsv = async () => {
    ...
    downloadCsv(
      ...
      rows.map((r) => [
        ...
        r.nombre_cliente,
        r.fecha || '',
        (r.total_neto ?? 0).toFixed(2),
        (r.impuesto ?? 0).toFixed(2),
        (r.descuento ?? 0).toFixed(2),
        r.st_anulado === 'S' ? 'Anulado' : 'Activo',
        r.no_factura || '',
      ]),
      meta
    )
  ...
  const exportPdf = async () => {
    ...
    <tbody>${rows
      .map(
        (r) => `<tr class="${r.st_anulado === 'S' ? 'anulado' : ''}">
    ...
    <section className='space-y-4'>
      <div className='flex flex-wrap items-center justify-between gap-3'>
        ...
        <div className='flex gap-2'>
          ...
          <Button variant='outline' size='sm' onClick={openListadoPdf}>
            <FileText className='mr-1 h-4 w-4' /> Imprimir PDF
          </Button>
          ...
          {!loading && rows.length === 0 && (
            <TableRow>
              <TableCell
                ...
                className='py-10 text-center text-muted-foreground'
              >
                No hay conduces para este periodo.
              </TableCell>
            </TableRow>
          )}
          {rows.map((row) => (
            <TableRow
              key={`${row.tipo_conduce}-${row.no_conduce}`}
              className={`cursor-pointer hover:bg-muted/50 ${row.st_anulado === 'S' ? 'line-through opacity-50' : ''}`}
            ...
            >
              ...
              <TableCell className='text-right font-mono'>
                {(row.total_neto ?? 0).toFixed(2)}
              </TableCell>
              <TableCell className='text-right font-mono'>
                {(row.impuesto ?? 0).toFixed(2)}
              </TableCell>
              <TableCell className='text-right font-mono'>
                {(row.descuento ?? 0).toFixed(2)}
              </TableCell>
              <TableCell className='text-center'>
                <Badge
                  variant={row.st_anulado === 'S' ? 'destructive' : 'default'}
                  className='text-xs'
                >
      ...
      {totalPages > 1 && (
        <div className='flex items-center justify-between text-sm text-muted-foreground'>
          <span>
            {total} registros &middot; Pagina {page} de {totalPages}
          </span>
          <div className='flex gap-1'>
            <Button
              variant='outline'
            ...
            <div className='flex-1 space-y-4 overflow-y-auto px-6 py-4'>
              ...
              <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
                ...
                <div>
                  ...
                  <p className='text-sm'>
                    {selected.tipo_moneda || 'RD'}{' '}
                    {selected.tasa_us ? `(${selected.tasa_us.toFixed(2)})` : ''}
                  </p>
                </div>
                <div>
                  <Label className='text-xs text-muted-foreground'>
                  ...
                  <TableBody>
                    ...
                    {selected.lineas.map((l) => (
                      <TableRow
                        key={l.no_linea}
                        className={
                          l.st_anulado === 'S' ? 'line-through opacity-40' : ''
                        }
                      >
                        ...
                        <TableCell className='text-right font-mono text-sm'>
                          {l.porc_descuento > 0 ? `${l.porc_descuento}%` : '—'}
                        </TableCell>
                        <TableCell className='text-right font-mono text-sm'>
                          {l.descuento > 0 ? fmtN(l.descuento) : '—'}
                        </TableCell>
                        <TableCell className='text-right font-mono text-sm'>
                          {l.itbis > 0 ? fmtN(l.itbis) : '—'}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
