...
type FacturaPendiente = {
  no_factura: string; tipo_factura: string; fecha: string; no_cliente: string
  total_neto: number; impuesto: number; total_bruto: number; asiento_cnt: string | null
}
...
export function GenerarAsientosFat({ noCia, punto, ano, mes }: Props) {
  ...
  const pendientes = rows.filter((r) => !r.asiento_cnt)
  const procesados = rows.filter((r) => !!r.asiento_cnt)
  ...
  return (
    <section className='space-y-4'>
      ...
      <div className='flex gap-4 text-sm'>
        ...
        <span className='text-muted-foreground'><span className='font-semibold'>{procesados.length}</span> ya generados</span>
      </div>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className='w-28 text-center'>Asiento CNT</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {loading && <TableRow><TableCell colSpan={8} className='py-10 text-center text-muted-foreground'>Cargando...</TableCell></TableRow>}
          {!loading && rows.length === 0 && <TableRow><TableCell colSpan={8} className='py-10 text-center text-muted-foreground'>Sin facturas en este período.</TableCell></TableRow>}
          {rows.map((row) => (
            <TableRow key={row.no_factura} className={row.asiento_cnt ? 'opacity-60' : ''}>
              ...
              <TableCell className='text-right font-mono'>{row.total_bruto.toFixed(2)}</TableCell>
              <TableCell className='text-center'>
                {row.asiento_cnt
                  ? <Badge variant='secondary' className='text-xs font-mono'>{row.asiento_cnt}</Badge>
