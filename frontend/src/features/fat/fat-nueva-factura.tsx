...
export function NuevaFactura({ noCia, punto }: Props) {
  ...
  const cargarCotizacion = async (no: string) => {
    ...
    if (!cot) {
      toast({
        title: 'Cotización/Conduce no encontrado',
        description: `Numero: ${num}`,
        variant: 'destructive',
      })
      return
    }
    ...
    if (cot.no_cliente) {
      try {
        const res = await regalGeneralApi.fatListClientes(
          noCia,
        ...
        /* sin cliente — el usuario lo carga */
      }
    }
    ...
    const nuevas: Linea[] = lineasCot.map((l) => ({
      ...
      monto:
        Number(l.cantidad || 0) *
        Number(l.precio || 0) *
        (1 - Number(l.porc_descuento || 0) / 100),
  ...
  useEffect(() => {
    ...
    regalGeneralApi
      ...
      .catch(() => {
        if (!cancelled) setProximoNcf(null)
      })
    return () => {
      cancelled = true
    }
  ...
  useEffect(() => {
    ...
    regalGeneralApi
      .fatProximoNoFactura(noCia, punto, tipoDoc)
      .then((d) => {
        if (!cancelled) setProximoNoFactura(d.prox_no_factura)
      })
      .catch(() => {
        if (!cancelled) setProximoNoFactura(null)
      })
    return () => {
      cancelled = true
    }
  ...
  const aplicarEmpaquesALinea = async (
    idx: number,
    noProdu: string,
    ...
    try {
      ...
      setLineas((prev) => {
        ...
        if (porDefecto && (!l.emp || l.emp === '—'))
          l.emp = porDefecto.descripcion || porDefecto.unidad
        arr[idx] = l
        return arr
      })
    } catch {
      /* sin empaques => UM queda como estática */
    }
  }
  ...
  const guardar = async () => {
    ...
    if (ncfInfo && ncfInfo.disponibles === 0) {
      toast({
        title: 'Error NCF',
        description: 'NCF agotado',
        variant: 'destructive',
      })
      return
    }
    if (proximoNcf && proximoNcf.agotado) {
      toast({
        title: 'Error NCF',
        description: `Serie ${proximoNcf.codigo_ncf} agotada — no hay NCF disponibles`,
        variant: 'destructive',
      })
      return
    }
      ...
      try {
        ...
        if (usado) {
          toast({
            title: 'NCF duplicado',
            description:
              'El próximo NCF ya está usado. Refrescá el formulario.',
            variant: 'destructive',
          })
          return
        }
    ...
    try {
      const res = await regalGeneralApi.fatCrearFactura({
        ...
        porc_pronto_pago: descProntoPago,
        no_condicion_pago: condicionPago,
    ...
    <div className='space-y-4 p-4' ref={formRef}>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-2'>
          <Button
            variant='outline'
            onClick={() => navigate({ to: '/fat/facturas' as never })}
          >
            Cancelar
          </Button>
        ...
        <div className='grid grid-cols-6 gap-3'>
          <div className='space-y-1'>
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') cargarCotizacion(noCotizacion)
              }}
            ...
            <div>
              ...
              <div className='flex items-center gap-2'>
                <span className='font-mono text-base font-bold'>
                  {ncfInfo.disponibles.toLocaleString()}
                </span>
                {ncfInfo.critical && (
                ...
                {!ncfInfo.critical && ncfInfo.low_stock && (
                  <Badge className='bg-yellow-500 text-xs text-white hover:bg-yellow-500'>
                    Stock Bajo
                  </Badge>
                )}
              </div>
      ...
      <div className='space-y-3 rounded-lg border bg-background p-4'>
        ...
        <div className='flex items-end gap-2'>
          ...
          {clienteSeleccionado ? (
            <div className='flex flex-1 flex-wrap items-center gap-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2'>
              ...
              {proximoNcf && (
                <div className='shrink-0'>
                  ...
                  <span
                    className={`font-mono font-bold ${proximoNcf.agotado ? 'text-red-600' : 'text-emerald-800'}`}
                  >
                    {proximoNcf.agotado
                      ? 'AGOTADO'
                      : proximoNcf.ncf_dgi_proximo}
                  </span>
                </div>
              )}
              {tipoDoc && proximoNoFactura !== null && (
                <div className='shrink-0'>
                  ...
                  <span className='font-mono font-bold text-emerald-800'>
                    {tipoDoc}-{String(proximoNoFactura).padStart(8, '0')}
                  </span>
                </div>
              )}
              <Button
            ...
            <div className='flex flex-1 items-center rounded-lg border border-dashed border-gray-300 px-3 py-2 text-sm text-gray-400'>
              {cargandoCliente
                ? 'Cargando cliente...'
      ...
      <div className='space-y-3 rounded-lg border bg-background p-4'>
          ...
          {!esContado && (
            <>
                ...
                >
                  ...
                  <SelectContent>
                    {condicionesPago.map((c) => (
                      <SelectItem
                        key={c.no_condicion_pago}
                        value={c.no_condicion_pago}
                      >
                        {c.descripcion}
                      </SelectItem>
                    ))}
              ...
              <div className='space-y-1'>
                ...
                <Input
                  type='number'
                  value={descProntoPago}
                  onChange={(e) =>
                    setDescProntoPago(parseFloat(e.target.value) || 0)
                  }
                  min={0}
                  max={100}
                  step='0.01'
        ...
        <div className='flex items-center gap-2'>
          <Checkbox
            id='itbis-en-precio'
            checked={itbisEnPrecio}
            onCheckedChange={(v) => setItbisEnPrecio(v === true)}
          />
          <Label
            htmlFor='itbis-en-precio'
            className='cursor-pointer font-normal'
          >
            ITBIS incluido en el precio de venta
          </Label>
        </div>
      </div>
      ...
      <div className='space-y-3 rounded-lg border bg-background p-4'>
        <div className='flex items-center justify-between'>
          ...
          <Button size='sm' onClick={agregarLinea} disabled={!tipoDoc}>
            + Agregar Linea
          </Button>
        </div>

        {!tipoDoc && (
          <p className='rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-600'>
              ...
              {lineas.map((linea, idx) => (
                <TableRow key={linea.id} className='hover:bg-background'>
                  <TableCell className='p-1'>
                    {almacenes.length > 0 ? (
                      <Select
                        value={linea.almacen || '__none__'}
                        onValueChange={(v) =>
                          updateLinea(idx, 'almacen', v === '__none__' ? '' : v)
                        }
                      >
