...
type Conduce = {
  ...
  no_cliente: number; nombre_cliente: string; fecha: string
  total_neto: number; total_linea: number; impuesto: number; descuento: number
  st_anulado: string; clase: string; tipo_factura: string; no_factura: string; vendedor: string
}
    ...
    <section className='space-y-4'>
      ...
      <Table>
        ...
        <TableBody>
          ...
          {!loading && rows.length === 0 && <TableRow><TableCell colSpan={10} className='py-10 text-center text-muted-foreground'>No hay conduces para este periodo.</TableCell></TableRow>}
          {rows.map((row) => (
      ...
      {totalPages > 1 && (
        <div className='flex items-center justify-between text-sm text-muted-foreground'>
          <span>{total} registros · Pagina {page} de {totalPages}</span>
          <div className='flex gap-1'>
