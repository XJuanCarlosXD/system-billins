...
function newRow(almacen = ''): DevRow {
  return {
    id: rowIdCounter++, noProdu: '', nombre: '', cantidad: '', costo: '',
...
export function DevolucionSuplidores({ noCia, punto }: Props) {
  ...
  const cargarOrigenWith = useCallback(async (tipo: string, no: string) => {
    if (!tipo || !no.trim()) {
    ...
    try {
      ...
      toast.success(`Se cargaron ${norm.length} línea${norm.length === 1 ? '' : 's'} del documento`)
    } catch (e: any) {
      const msg = e?.message || ''
  ...
  const cargarOrigen = useCallback(() => cargarOrigenWith(tipoDocOrigen, docOriginal), [cargarOrigenWith, tipoDocOrigen, docOriginal])
  ...
  const cargarEmpaques = useCallback(async (idx: number, noProdu: string, costoBase: number) => {
    try {
      ...
      setRows(prev => {
        ...
        arr[idx] = { ...arr[idx], empaques: emps, empaque, costo: costoBase ? (costoBase * factor).toFixed(4) : arr[idx].costo }
        return arr
      })
    } catch { /* sin empaques */ }
  }, [])
  ...
  const selectFromOrigen = (linea: LineaOrigen) => {
    ...
    const yaUsada = rows.some((r, i) => i !== idx && r.origenLinea && r.origenLinea === linea.no_linea)
    if (yaUsada) {
  ...
  const handleSave = async () => {
    ...
    if (!proveedor.trim()) { toast.error('Seleccione el proveedor'); return }
    if (!lineasOrigen.length) { toast.error('Cargue el documento original a devolver'); return }
    ...
    for (const r of validRows) {
      if (r.origenCantidad != null && (parseFloat(r.cantidad) || 0) > r.origenCantidad) {
        toast.error(`Cantidad a devolver (${r.cantidad}) excede la original (${r.origenCantidad}) en ${r.noProdu}`)
        return
      }
    }
    ...
    <TooltipProvider>
      <section className='space-y-6'>
        <div>
          ...
          <p className='text-sm text-muted-foreground'>FINV205 — Devolución de mercancía a proveedores (afecta documento original)</p>
        </div>

        {/* Documento original a afectar */}
        <Card>
          <CardHeader>
            <CardTitle className='text-base'>Documento Original a Devolver</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className='space-y-1'>
                ...
                <div className='flex gap-1'>
                  <Input
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') cargarOrigen() }}
                  />
                  <Button
                    type='button'
                    variant='outline'
              ...
              {headerOrigen && (
                <>
                  <div className='space-y-1 col-span-2'>
                    <Label className='text-xs text-muted-foreground'>Documento cargado</Label>
                    <div className='h-9 flex items-center px-3 rounded-md border bg-muted text-xs gap-2'>
                      ...
                      <span className='truncate'>
                        {headerOrigen.proveedor_nombre || headerOrigen.cliente_nombre || '—'}
                      </span>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className='w-[110px] text-right'>Cant. Original</TableHead>
      ...
      <Dialog open={pickerOpen} onOpenChange={(o) => { if (!o) { setPickerOpen(false); setPickerForIdx(null) } }}>
        <DialogContent className='w-[80vw] max-w-none sm:max-w-none max-h-[80vh] flex flex-col p-0 gap-0 overflow-hidden'>
          <DialogHeader className='px-6 py-4 border-b shrink-0'>
            <DialogTitle>
              Líneas del documento {tipoDocOrigen}-{String(docOriginal).padStart(7, '0')}
            </DialogTitle>
            <p className='text-xs text-muted-foreground'>Selecciona la línea del producto a devolver</p>
          </DialogHeader>
                ...
                {lineasOrigen.map((l) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className='text-right font-mono text-sm tabular-nums'>{fmt(l.costo)}</TableCell>
                      <TableCell className='text-right font-mono text-sm tabular-nums font-semibold'>
                        {fmt((l.monto_neto && l.monto_neto > 0) ? l.monto_neto : l.cantidad * l.costo)}
                      </TableCell>
          ...
          <div className='px-6 py-3 border-t shrink-0 bg-gray-50 flex items-center justify-between text-sm text-gray-500'>
            <span>{lineasOrigen.length} línea{lineasOrigen.length === 1 ? '' : 's'} en el documento</span>
