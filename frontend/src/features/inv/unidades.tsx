...
export function UnidadesMedida(_props: Props) {
  ...
  const load = () => {
    ...
    regalGeneralApi
      ...
      .catch((err) => setError(err?.detail?.error ?? err?.message ?? 'Error al cargar unidades de medida'))
  ...
  const openEdit = (row: UnidadMedida) => { setEditing(row); setFormOpen(true) }

  const confirmDelete = async () => {
    if (!deleting) return
    try {
      await regalGeneralApi.invDeleteUnidad(deleting.cod_unidad)
      toast.success(`Unidad ${deleting.cod_unidad} eliminada`)
      setDeleting(null)
      load()
    } catch (err: any) {
        ...
        <Dialog open onOpenChange={() => setDeleting(null)}>
          <DialogContent className='max-w-sm'>
            ...
            <p className='text-sm text-muted-foreground'>
              Se eliminará la unidad <span className='font-mono font-medium'>{deleting.cod_unidad}</span> ({deleting.descripcion}). Esta acción no se puede deshacer.
            </p>
...
function UnidadFormDialog({
  ...
  const [codigo, setCodigo] = useState(unidad?.cod_unidad ?? '')
  ...
  const save = async () => {
    ...
    try {
      if (isEdit) {
        await regalGeneralApi.invUpdateUnidad(codigo.trim(), { descripcion: descripcion.trim() })
        toast.success(`Unidad ${codigo.trim()} actualizada`)
      } else {
        await regalGeneralApi.invCreateUnidad({ unidad: codigo.trim(), descripcion: descripcion.trim() })
        toast.success(`Unidad ${codigo.trim()} creada`)
      }
      onSaved()
    } catch (err: any) {
